...
func (m *Module) RegisterTasks(tasks *asynq.ServeMux) {
	tasks.HandleFunc("billing:charge", m.handleCharge)
}

func (m *Module) handleCharge(ctx context.Context, task *asynq.Task) error {
	var payload chargePayload
	if err := web.DecodeTask(task.Payload(), &payload); err != nil {
		return err
	}
	return m.Charge(ctx, payload.InvoiceID)
}
