...
The default worker count is `min(GOMAXPROCS, 8)`; set `BIFROST_QUICKJS_WORKERS` only after load testing. The shared lazy SSR registry bundles all page components once per worker, so React evaluates once per worker instead of once per page and a large page count keeps memory flat, while import failures stay isolated to the selected page. On startup the host evaluates each SSR bundle on every worker (priming), so the first request does not pay the per-worker cold-start eval — without it, a fresh 8-worker server pays ~160 ms per worker on first use.

Garbage collection runs a full collection on every worker at render boundaries, every 25 renders by default (`BIFROST_QUICKJS_GC_INTERVAL` overrides the cadence; `0` restores threshold mode). Render-boundary collection keeps the automatic-GC pause out of the request stream: sweep + soak measurements on the heavy bench page show cadence 5-25 beating the 16 MiB threshold on solo TTFB p50 (~5%) and burst p95 (549-616 ms vs up to 974 ms in threshold mode), at 1.6-2.5x peak RSS — garbage accumulates between boundary collections and grows roughly linearly with the interval and with soak length and page weight (threshold mode: ~0.9 GB; cadence 25: 1.6-2.4 GB on this deliberately heavy 4.6 MB fixture; real pages scale proportionally; `gcInt=1` keeps RSS at ~0.45 GB but costs ~5% request latency). Threshold mode bounds peak RSS to the working set plus the threshold — quickjs-go disables automatic GC by default (`-1`), which leaks per-render garbage until OOM (measured ~150 MB/s under load). `BIFROST_QUICKJS_GC_THRESHOLD` overrides the threshold in bytes; `-1` restores the library default. Memory-bound deployments with small pages should set `BIFROST_QUICKJS_GC_INTERVAL=0`.
