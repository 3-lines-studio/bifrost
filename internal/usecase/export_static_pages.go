...
func ExportStaticPages(in ExportStaticPagesInput) error {
	...
	for _, route := range in.Routes {
		...
		for _, entry := range entries {
			...
			if !staticPathMatchesRoute(route.Pattern, normalizedPath) {
				exportErrors = append(exportErrors, fmt.Errorf(
					"static page %s: output path %q does not match its route pattern",
					route.Pattern,
					normalizedPath,
				))
				continue
			}
			owner := route.Pattern + " (" + config.ComponentPath + ")"
			if previous, exists := outputOwners[normalizedPath]; exists {
				exportErrors = append(exportErrors, fmt.Errorf(
					"static page %s: output path %q is already generated by %s",
					route.Pattern,
					normalizedPath,
					previous,
				))
			...
			outputOwners[normalizedPath] = owner
...
func staticPathMatchesRoute(pattern string, outputPath string) bool {
	if !strings.HasPrefix(pattern, "/") || strings.ContainsAny(pattern, " ?#") {
		return false
	}

	subtree := pattern != "/" && strings.HasSuffix(pattern, "/")
	pattern = path.Clean(pattern)
	if pattern == "/{$}" {
		pattern = "/"
	}
	patternParts := strings.FieldsFunc(pattern, func(r rune) bool { return r == '/' })
	outputParts := strings.FieldsFunc(outputPath, func(r rune) bool { return r == '/' })

	for i, part := range patternParts {
		if part == "{$}" {
			return i == len(patternParts)-1 && len(outputParts) == i
		}
		if part == "*" || (strings.HasPrefix(part, "{") && strings.HasSuffix(part, "...}")) {
			return len(outputParts) > i
		}
		if len(outputParts) <= i {
			return false
		}
		if (strings.HasPrefix(part, "{") && strings.HasSuffix(part, "}")) || strings.HasPrefix(part, ":") {
			continue
		}
		if part != outputParts[i] {
			return false
		}
	}
	if subtree {
		return len(outputParts) >= len(patternParts)
	}
	return len(outputParts) == len(patternParts)
}
