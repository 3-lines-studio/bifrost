...
func TestScanFileForPagesRejectsInvalidOptionContracts(t *testing.T) {
	tests := []struct {
		...
		{name: "streaming shell on client page", opts: `bifrost.WithClient(), bifrost.WithStreamingShell(".s{}")`},
		{name: "preloads on static page", opts: `bifrost.WithStatic(), bifrost.WithPreloads(bifrost.Preload{})`},
		{name: "prerender on client page", opts: `bifrost.WithClient(), bifrost.WithPrerender("/next")`},
		{name: "nil streaming shell", opts: `bifrost.WithStreamingShell(nil)`},
