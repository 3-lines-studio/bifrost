...
func writeServerEntry(path string, componentImport string) error {
	...
	var buf bytes.Buffer
	if err := ServerEntryTemplate.Execute(&buf, map[string]string{
		"ComponentImport": componentImport,
	}); err != nil {
		return err
	}
...
func getAssetsFromManifest(man *buildManifest, entryName string) (scriptSrc, cssHref string, chunks []string, isStatic bool, ssrPath string) {
